...
consumer module. With fewer than two arguments, or `--import` or `--module` given no name, or a
source path that cannot be read, the tool SHALL write an error to standard error and exit `1`.
...
deduplicated. An import inside an `#if` block SHALL be rewritten in place with the same rule and the
block emitted as it stands.
...
followed by the normalised imports (always including `import WireMVC`), then one `extension
_WireRouteContributor_<Controller>: RouteContributor` per `@Controller` sorted by controller name,
then any typed clients sorted by controller name, then the bootstrap sources. An input with no
`@Controller` and no `@WireMVCBootstrap` SHALL produce the header and imports only.
...
Pinned by: `Tests/WireMVCCodegenTests/ControllerClientGenerationTests.swift` (`theClientIsOnlyEmittedForATestConsumer`).

### Requirement: The keyed harness adds `import WireTesting` only when a `TestingKey` is served
Under `--test-entry`, WireMVCRouteGen SHALL discover at most one `TestingKey` attributed to the
consumer module and, when one is found, SHALL add `import WireTesting` and emit the keyed harness
beside the keyless entry. A keyless test consumer SHALL NOT import `WireTesting`.
...
Pinned by: `Tests/WireMVCCodegenTests/RouteContributorGenerationTests.swift` (`keyedHarnessImportsWireTestingAndKeylessDoesNot`, `aSecondTestingKeyIsRejected`, `aDependencysTestingKeyIsNotServed`).
...
Pinned by: nothing yet in this repository. The consumer is specified in [wire-open-api build-plugins-and-gen-cli](https://github.com/swift-wire/wire-open-api/blob/main/openspec/specs/build-plugins-and-gen-cli/spec.md).
...
the executables and test targets in `Fixtures/Package.swift` SHALL apply `WireMVCBuildPlugin`, and
