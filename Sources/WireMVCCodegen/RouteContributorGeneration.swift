...
private let witnessSignature = """
    registerWireRoutes<Builder: HTTPServerRouteBuilder>(
...
/// final generated source. Imports are canonicalised (see ``normalizedImports``) and, like the
/// extensions, emitted in sorted order; the bootstrap sources (its generated `@main`/`.wiremvc()`
/// entry) come last, at module scope. Split out of `generateRouteContributors` to keep it within the
/// body-length budget.
private func assembleGeneratedSource(
    ...
    for line in normalizedImports(imports) {
        lines.append("")
        lines.append(line)
    }
...
/// (the controller's domain types, WireMVC's routing surface) stay in scope. The access level each was
/// written at is dropped at assembly (see ``normalizedImports``), because everything this generator
/// emits is internal.
