...
func bootstrapBuildLines(
    ...
    let registrations = [
        mountIntrospection, notFoundRegistration, renderMethodNotAllowedRegistration(), extraRegistrations,
    ]
    .filter { !$0.isEmpty }
    .joined(separator: "\n")
...
/// The `builder.registerMethodNotAllowed { … }` every bootstrap app gets — the `405` sibling of
/// ``renderNotFoundRegistration(bootstrap:)`` above, and for the same reason.
///
...
///
/// **Why this one takes no `bootstrap` and returns no diagnostics**, unlike its sibling: there is no
/// `@MethodNotAllowed` annotation to author a custom 405, so nothing varies per app and nothing can be
/// mis-declared. That is also why it is called inside ``bootstrapBuildLines`` rather than threaded in as a
/// parameter — a parameter exists for the 404 because its callers must surface the diagnostics authoring
/// one can produce. When an annotation arrives, this gains both and becomes a parameter like the 404.
...
func bootstrapAppCodingExpression(bootstrap: ControllerDeclaration) -> String {
    ...
    guard
        let key = RouteBlockGenerator(subjectAccessor: "", factoryKeys: [], globalErrorMappings: [])
            .codingKey(from: bootstrap.attributes)?.reference
    ...
    return "graph.\(proxyProperty).\(codingProxyField(for: key))"
}
